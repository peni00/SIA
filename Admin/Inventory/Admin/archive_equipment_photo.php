<?php
    ...
    if(isset($_POST['upload'])){
        ...
        $stmt = $conn->prepare("SELECT * FROM equiparchive WHERE id=?");
        ...
        try{
            $stmt = $conn->prepare("UPDATE equiparchive SET photo=? WHERE id=?");
            $stmt->bind_param('si', $new_filename, $id);
            $stmt->execute();

            $_SESSION['status'] = 'Equipments photo updated successfully';
